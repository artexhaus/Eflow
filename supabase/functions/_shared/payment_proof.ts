...
// supabase/migrations/20260924030000_protect_only_payment_proof.sql (minus the
// chatter check, which lives in chatter.ts). Keep the two in sync. Postgres
...
const PROOF_RE =
  /\b(receipts?|e-?receipt|payments? (received|confirmed|confirmation|successful|processed|complete|completed)|thanks? (you )?for your (payment|order|purchase)|you paid|you sent (a )?payment|sent you (a )?payment|has been paid|paid in full|(bill|invoice) (has been )?paid|order (confirm(ed|ation)|#|number|no\.?|receipt)|your ([\w.&'-]+ ){0,3}order|purchase confirm(ed|ation)|refund(ed)?|transaction (receipt|confirmation))\b/i;

// Reminders, notices and status updates - fluff once read or out of date.
const NOTICE_RE =
  /(remind|\bdue\b|past due|overdue|upcoming|scheduled|auto-?pay|\bis (ready|available)|now available|ready to view|view (your|online)|don't forget|expir|renew|shipped|out for delivery|delivered|on (its|the) way|arriving|\btrack(ing)?\b|low balance|declined|failed|action required|your (next|first) order)/i;
